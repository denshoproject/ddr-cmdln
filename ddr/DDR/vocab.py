...
def _get_vocab_fs(base_dir, vocab):
    """Loads vocabulary data from filesystem.
    """
    path = os.path.join(base_dir, vocab)
    if not '.json' in path:
        path = '%s.json' % path
...
def _get_vocab_http(base_url, vocab):
    """Loads vocabulary data from vocab API.
    """
    url = urlparse.urljoin(base_url, '%s.json' % vocab)
...
def get_vocab(base, vocab):
    ...
    """
    if 'http://' in base:
        return _get_vocab_http(base, vocab)
    return _get_vocab_fs(base, vocab)

def _get_vocabs_all_fs(base, exclude='index'):
    return {
        vocab.replace('.json',''): _get_vocab_fs(base, vocab)
